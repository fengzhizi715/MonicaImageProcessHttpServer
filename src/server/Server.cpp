...
void session::handle_request() {
    try {
        ...
        if (!router_.handleRequest(method, target)) {
...
void session::process_image(const std::string& target, std::function<Mat(const Mat&)> processor) {
    try {
...
void session::initializeRoutes() {
    ...
    // Version endpoint
    router_.addRoute("GET", "/version", [this]() {
    ...
    router_.addRoute("POST", "/api/sketchDrawing", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Sketch drawing request received");
        process_image("/api/sketchDrawing", [this](Mat src) { return globalResource_->processSketchDrawing(src); });
    });
    
    // Face detection endpoint
    router_.addRoute("POST", "/api/faceDetect", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Face detection request received");
        process_image("/api/faceDetect", [this](Mat src) { return globalResource_->processFaceDetect(src); });
    });
    
    // Face landmark endpoint
    router_.addRoute("POST", "/api/faceLandMark", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Face landmark request received");
        process_image("/api/faceLandMark", [this](Mat src) { return globalResource_->processFaceLandMark(src); });
    });
    
    // Face swap endpoint
    router_.addRoute("POST", "/api/faceSwap", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Face swap request received");
        handleFaceSwap();
    });
    
    // Cartoon endpoint
    router_.addRoute("POST", "/api/cartoon", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Cartoon request received");
        handleCartoon();
    });
    
    // Face beauty endpoint
    router_.addRoute("POST", "/api/faceBeauty", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Face beauty request received");
        handleFaceBeauty();
    });
    
    // Change person background endpoint
    router_.addRoute("POST", "/api/changePersonBackground", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Change person background request received");
        handleChangePersonBackground();
    });
    
    // Change hair color endpoint
    router_.addRoute("POST", "/api/changeHairColor", [this]() {
        LOG_REQ(AixLog::Severity::info, requestMetrics_, "Change hair color request received");
        handleChangeHairColor();
    });
}

void session::handleFaceSwap() {
    try {
...
void session::handleChangePersonBackground() {
    try {
        // Parse multipart/form-data
        auto parts = parseMultipartFormDataManual(req_);
...
void session::handleChangeHairColor() {
    try {
        std::string target = std::string(req_.target());
        // Parse path + query parameters using parse_relative_ref
        auto targetRes = boost::urls::parse_relative_ref(target);
